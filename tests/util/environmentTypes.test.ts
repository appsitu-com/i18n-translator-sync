...
`;
  ...
  it('does not create file when it already exists', async () => {
    resetEnvInitialization();
    const files: Record<string, string> = {
      '/test/workspace/.translator.env': 'existing content'
    };
    const mockFs = createMockFileSystem(files);
    ...
    const envFileCalls = writeCalls.filter((call: any) => call[0].path.endsWith('.translator.env'));
  ...
  it('adds .translator.env to .gitignore', async () => {
    ...
    expect(writeCall[1]).toContain('.translator.env');
  });
});
