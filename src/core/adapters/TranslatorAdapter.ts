...
import { IWorkspaceWatcher } from '../util/watcher';
import { JsonlTranslationMemory, ITranslationMemory } from '../tm/ITranslationMemory';
import { IConfigProvider } from '../coreConfig';
...
export interface ITranslatorManager {
  ...
  bulkTranslate(
    ...
    progress?: (processed: number, total: number, currentFile: string) => void,
    force?: boolean
  ): Promise<number>
  dispose(): void
}

type TranslationMemoryFactory = (dbPath: string, workspacePath: string, logger: ILogger) => ITranslationMemory

type TranslatorManagerFactory = (params: {
...
type TranslatorAdapterDependencies = {
  createTranslationMemory?: TranslationMemoryFactory
  createTranslatorManager?: TranslatorManagerFactory
}
...
export abstract class TranslatorAdapter {
  protected translatorManager?: ITranslatorManager;
  protected tm?: ITranslationMemory;
  protected running = false;
  protected translatorEngines?: ITranslatorEngines;
  protected translatorConfig?: ITranslatorConfig;
  protected readonly dependencies: TranslatorAdapterDependencies;
  ...
  constructor(
    ...
    dependencies: TranslatorAdapterDependencies = {}
  ) {
   ...
   */
  protected abstract createWatcher(): IWorkspaceWatcher;
   ...
   */
  protected async getTm(dbMustExist = false): Promise<ITranslationMemory | undefined> {
    ...
    this.tm =
      this.dependencies.createTranslationMemory?.(dbPath, this.workspacePath, this.logger) ??
      new JsonlTranslationMemory(dbPath, this.workspacePath, this.logger);
    ...
    try {
      ...
      if (!this.translatorManager) {
        ...
        this.translatorManager =
          this.dependencies.createTranslatorManager?.({
